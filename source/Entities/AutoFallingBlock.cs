        ...
        private void LandParticles() {
            for (int i = 2; i <= base.Width; i += 4) {
                if (base.Scene.CollideCheck<Solid>(base.BottomLeft + new Vector2(i, 3f))) {
                    SceneAs<Level>().ParticlesFG.Emit(FallingBlock.P_FallDustA, 1, new Vector2(base.X + i, base.Bottom), Vector2.One * 4f, -(float)Math.PI / 2f);
                    float direction = (!(i < base.Width / 2f)) ? 0f : ((float)Math.PI);
                    SceneAs<Level>().ParticlesFG.Emit(FallingBlock.P_LandDust, 1, new Vector2(base.X + i, base.Bottom), Vector2.One * 4f, direction);
                }
            }
        }
